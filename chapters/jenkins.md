...
You could run a jenkins container on your docker host by using official jenkins image with the version `2.178-Slim`. Use following command to run a jenkins container,
```
...
![](./images/jenkins4.png)
* Next goto build and choose execute shell to run following commands, once you complete the commands, save & apply to build your job.
...
* After build succesful, you could see the jar file created on your workspace. Example is given following , you can verify your workspace by using it.
...
* goto your `worker-build` job configuration page and choose `poll SCM` under build triggers. In that poll scm mention following time interval for periodically poll the git repository, if there is any change in the git repository it will build your job.
```
H/2 * * * *
```
...
* Copy the api token some where and save the configuration. You could run following command to trigger the job from CLI.
```
...
Now you are going to setup pipeline view for this build jobs, for that you need to install `build pipeline` plugin from manage jenkins page. follow the following steps to setup pipeline.
...
Follow the following steps to setting up the webhook
...
* Again goto your github repository and choose `settings -> webhooks`, add payload url and content type as application/json, save it(refer example image given following).
